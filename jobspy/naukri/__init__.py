...
class Naukri(Scraper):
    """
    Naukri.com job scraper for India-focused job searches.
    ...
    def __init__(
        ...
        # Add random request ID to headers to avoid detection
    ...
    def _update_request_id(self):
        """Update request ID header with random value to avoid detection"""
        import uuid
        request_id = str(uuid.uuid4()).replace("-", "")
        self.session.headers["x-request-id"] = request_id
        self.session.headers["x-correlation-id"] = request_id[:16]
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        Returns:
            JobResponse containing list of JobPost objects
        """
        ...
        while continue_search():
            ...
            try:
                ...
                # Handle 406 Recaptcha required error
                if response.status_code == 406:
                    log.error(f"Naukri API returned 406 - Recaptcha required. This may indicate rate limiting or detection.")
                    log.error(f"Consider using proxies or reducing request frequency.")
                ...
                if response.status_code not in range(200, 400):
                    ...
                    # If we get consecutive errors, stop scraping
                    if request_count > 3 and len(job_list) == 0:
                        log.error("Multiple failed requests, stopping scrape")
                        return JobResponse(jobs=job_list)
                ...
                log.error(f"Naukri API request timed out after 15 seconds")
                return JobResponse(jobs=job_list)
            ...
            for job in job_details:
                ...
                try:
                    ...
                    # Don't raise exception, continue with next job
                    continue
    ...
    def _process_job(
        ...
        Args:
            job: Job dictionary from API response
            job_id: Unique job identifier
            full_descr: Whether to fetch full description
    ...
    def _get_location(self, placeholders: list[dict]) -> Location:
        ...
        Args:
            placeholders: List of placeholder dictionaries from API
    ...
    def _parse_date(self, label: str, created_date: int) -> Optional[date]:
        ...
        Args:
            label: Date label string (e.g., "today", "5 days ago")
            created_date: Unix timestamp in milliseconds
    ...
    def _infer_work_from_home_type(self, placeholders: list[dict], title: str, description: str) -> Optional[str]:
        ...
        Returns:
            Work from home type string or None
        """
